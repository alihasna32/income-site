...
import { Banknote, Check, Loader2, ShieldCheck, X } from "lucide-react";
...
const METHODS = {
  ...
  mobile_wallet: { label: "Mobile banking" },
  paypal: { label: "PayPal" },
};

export function AdminTakaWithdrawalsTable() {
  ...
  const load = useCallback(async () => {
    try {
      const res = await fetch("/api/admin/taka-withdrawals", { cache: "no-store" });
      if (res.ok) {
        const data = await res.json();
        setRows(data.withdrawals || []);
      }
    } catch {
      toast("Could not load Taka withdrawals", "error");
    } finally {
      setLoading(false);
    }
  }, [toast]);
  ...
  const act = async (id, action) => {
    setActingId(id);
    try {
      const res = await fetch(`/api/admin/taka-withdrawals/${id}`, {
        method: "PATCH",
      ...
      if (res.ok) {
        toast(
          action === "approve"
            ? "Taka withdrawal approved"
            : action === "reject"
            ? "Taka withdrawal rejected"
            : "Taka withdrawal marked complete",
          "success"
        );
        setRejectNote("");
        load();
  ...
  const resolved = rows.filter((r) => r.status !== "pending" && r.status !== "approved");
      ...
      <tr key={w.id} className="align-top">
          ...
          <span className="flex items-center gap-2.5">
            <span
              className={cn(
                ...
                w.profile?.avatar_emoji
                  ? ""
                  : `bg-gradient-to-br ${avatarGradient(w.profile?.display_name || w.profile?.username || "u")}`
              )}
            >
              {w.profile?.avatar_emoji ||
                initials(w.profile?.display_name || w.profile?.username || "U")}
            </span>
            <span>
              <span className="block font-semibold text-plum whitespace-nowrap">
                {w.profile?.display_name || "Unknown"}
              </span>
              <span className="block text-xs text-muted">
                @{w.profile?.username || "—"} · {w.userId?.slice(0, 8)}…
              </span>
            </span>
          </span>
        </td>
        <td>
          <span className="font-bold text-plum whitespace-nowrap">
            ৳{new Intl.NumberFormat("en-US").format(w.amount)}
          </span>
              ...
              {pending.length ? (
                ...
                <tr>
                  <td colSpan={6} className="py-10 text-center text-sm text-muted">
                    ...
                    No pending Taka withdrawals to review.
                  </td>
                </tr>
              )}
            </tbody>
      ...
      {approved.length > 0 && (
        <section className="card bg-base-100 border border-base-300 shadow-card overflow-hidden">
          <div className="px-5 pt-5">
            <h2 className="font-bold text-plum">
              Approved — ready to complete{" "}
              <span className="badge badge-sm ml-1 bg-info/15 text-info">
                {approved.length}
              </span>
            </h2>
            <p className="mt-1 text-sm text-muted">
              Mark these as "complete" once the payout has been sent to the user.
            </p>
          </div>
          <div className="overflow-x-auto">
            <table className="table table-sm sm:table-md">
              ...
              <tbody>{renderRows(approved)}</tbody>
            </table>
          </div>
        </section>
      )}
