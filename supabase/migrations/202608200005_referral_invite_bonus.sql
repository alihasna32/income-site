-- Referral invitee bonus: a new user joining with a friend's code earns coins,
-- and the referrer keeps their own bonus. Both get a notification.
-- Admin-controllable via admin_settings key 'referrals'.
...
select 'referrals', '{"bonus_coins":30,"invite_bonus_coins":60}'
where not exists (select 1 from public.admin_settings where key = 'referrals');
...
begin
  ...
  if v_ref_code is not null then
    ...
    if v_referrer.id is not null then
      select
        coalesce(nullif(value ->> 'bonus_coins', ''), '30')::int,
        coalesce(nullif(value ->> 'invite_bonus_coins', ''), '60')::int
      ...
      insert into public.referrals (
        ...
        v_referrer.id, new.id, v_referral_bonus, 'credited'
      )
      ...
      if v_referral.id is not null then
        ...
        perform public.credit_reward(
          ...
          v_invite_bonus,
          10,
          'Referral bonus — you joined with a friend',
          'referral-invite:' || v_referral.id,
          jsonb_build_object('referral_id', v_referral.id)
        );
        ...
        values (
          ...
          'You joined with a friend — ' || v_invite_bonus || ' coins added.'
        );
      end if;
    end if;
  end if;
